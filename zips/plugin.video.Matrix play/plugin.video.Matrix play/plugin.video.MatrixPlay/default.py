...
### link da pagina principal
menu_principal = 'https://pastebin.com/raw/e2mqds3H
try:
    origem = menu_principal.decode('base64')
except:
    origem = menu_principal
## user agent da origem
useragent = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.88 Safari/537.36'
####################### SKINDEX ##############################################
Skindex_menu = 'true'
####  BOAS VINDAS DO ADDON  ##################################################
welcome_message = '[B][COLOR blue]||||||| ADDON Matrix Play |||||||[/COLOR][/B]'
##############################################################################
...
welcome_description = Filmes e Series muito mais '##############################################################################
## configuração do multilink
multlinkdialog = '[COLOR white]Selecione um item:[/COLOR]'
multilinktag = '$nome='
color_multilink = 'blue'
name_multilink = 'itens'

###favoritos
addtofav = 'Adicionar aos Favoritos do '+AddonTitle
removefromfav = 'Remover dos Favoritos do '+AddonTitle
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[%s Lists-%s]: %s" %(AddonTitle,addon_version,string))	
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification("+AddonTitle+",We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification("+AddonTitle+",We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")


def SKindex():
    addon_log("SKindex")
    #addDir(name,url,mode,iconimage,fanart,description,genre,date,credits)
	...
	try:
		if os.path.exists(favorites) == True:
			...
			if FAV == "[]":
				...
				addDir('[COLOR blue][B]Meus Favoritos[/B][/COLOR]','url',4,os.path.join(home, 'fanart.jpg'),FANART,'','','','')
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification("+AddonTitle+",New source added.,5000,"+icon+")")
        if not url is None:
...
def getSoup(url,data=None):
        ...
        if url.startswith('http://') or url.startswith('https://'):
            ...
            try:  
                ...
                if '+dfa47aa6bb' in data:
                    data = data.split('+dfa47aa6bb')
...
def getFavorites():
        ...
        for i in items:
            ...
            if i[4] == 0:
                addLink(url.encode('utf-8'),name.encode('utf-8', 'ignore'),iconimage,fanArt,'','','','fav',playlist,regexs,total)
            else:
                addDir(name.encode('utf-8', 'ignore'),url.encode('utf-8'),i[4],iconimage,fanart,'','','','','fav')
...
def play_playlist(name, mu_playlist,queueVideo=None):
        ...
        if addon.getSetting('ask_playlist_items') == 'true' and not queueVideo :
            ...
            for i in mu_playlist:
                #if '$$lsname=' in i:
                if multilinktag in i:
                    #d_name=i.split('$$lsname=')[1].split('&regexs')[0]
                    d_name=i.split(multilinktag)[1].split('&regexs')[0]
                    names.append(d_name)                                       
                    #mu_playlist[iloop]=i.split('$$lsname=')[0]+('&regexs'+i.split('&regexs')[1] if '&regexs' in i else '')                    
                    mu_playlist[iloop]=i.split(multilinktag)[0]+('&regexs'+i.split('&regexs')[1] if '&regexs' in i else '')                    
                else:
            ...
            index = dialog.select(multlinkdialog, names)
            if index >= 0:
...
def download_file(name, url):
        ...
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno(AddonTitle, 'Do you want to add this file as a source?')
        if ret:
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':
                ...
                contextMenu.append((removefromfav,'XBMC.RunPlugin(%s?mode=6&name=%s)'
            ...
            if not name_fav in FAV:
                ...
                contextMenu.append((addtofav,'XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
#                print 'setting as folder in link'
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    (removefromfav,'XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
            elif not name_fav in FAV:
                try:
                    fav_params = (
                        '%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=0'
                        %(sys.argv[0], urllib.quote_plus(name), urllib.quote_plus(url), urllib.quote_plus(iconimage), urllib.quote_plus(fanart))
                ...
                if regexs:
                    fav_params += "&regexs="+regexs
                contextMenu.append((addtofav,'XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
#            ln+='</item>'
        ...
        if url:
            ...
            xbmc.executebuiltin("XBMC.Notification("+AddonTitle+",Failed to extract regex. - "+"this"+",4000,"+icon+")")
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification("+AddonTitle+",Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification("+AddonTitle+",Parental Block Disabled,5000,"+icon+")")
        else:
            xbmc.executebuiltin("XBMC.Notification("+AddonTitle+",Wrong Pin??,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))

elif mode==56:
    addon_log("disable lock")
    addon.setSetting('parentalblocked', "true")
    xbmc.executebuiltin("XBMC.Notification("+AddonTitle+",Parental block enabled,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
